...
execute if data entity @s interaction if score @n[tag=_game.LabDoor.Lab2] door.IsOpen matches 1 as @n[tag=aj.ld.root] run function animated_java:ld/animations/enter/play
execute if data entity @s interaction if score @n[tag=_game.LabDoor.Lab2] door.IsOpen matches 0 as @n[tag=aj.ld.root] run function animated_java:ld/animations/exit/play
